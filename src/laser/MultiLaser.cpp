    ...
    for ( amrex::MFIter mfi(m_slices, DfltMfiTlng); mfi.isValid(); ++mfi ){
        ...
        for (int ilaser = 0; ilaser < m_nlasers; ilaser++) {
            ...
            if (laser.m_laser_init_type == "parser") {
                ...
                const amrex::Real y_slope_z_foc = laser.m_y_slope_z_foc;
                amrex::ParallelFor(
                bx,
                [=] AMREX_GPU_DEVICE(int i, int j, int k)
                {
                    ...
                    Complex diffract_factor = 1._rt + I * (zp - zfoc * (1 + y_slope_z_foc * yp )+ z0 * std::cos(propagation_angle_yz)) \
                       * 2._rt/(k0 * w0_2);
                    ...
                    Complex stretch_factor = 1._rt \
                        + 4._rt * (-zeta + beta * zfoc * (1 + y_slope_z_foc * yp )) * inv_tau2 * (-zeta + beta * zfoc *(1 + y_slope_z_foc * yp )) * inv_complex_waist_2 \
                        + 2._rt * I * (phi2 - beta * beta * k0 * zfoc * (1 + y_slope_z_foc * yp )) * inv_tau2;
                    ...
                    Complex time_exponent = 1._rt / ( stretch_factor * L0 * L0 ) *
                        ...
                        * (-zeta - beta * zfoc * (1 + y_slope_z_foc * yp)) * clight * inv_complex_waist_2, 2);
