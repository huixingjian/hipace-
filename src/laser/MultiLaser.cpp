            ...
            if (laser.m_laser_init_type == "parser") {
                ...
                [=] AMREX_GPU_DEVICE(int i, int j, int k)
                {
                    ...
                    Complex time_exponent = 1._rt / ( stretch_factor * L0 * L0 ) *
                        amrex::pow(zp - beta * k0 * (x + x0 + yp + y0 * std::cos ( propagation_angle_yz + PFT_yz )) * clight  - 2._rt * I *
                        (x + x0 + yp + y0 * std::cos( propagation_angle_yz + PFT_yz)) *(zeta - beta * zfoc) * clight * inv_complex_waist_2, 2);
